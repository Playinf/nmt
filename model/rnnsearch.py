...
# fast version of rnnsearch, experimental
def fast_rnnsearch_config():
    opt = {}
    ...
    # encoder
    opt['encoder/forward-rnn/variant'] = 'fast'
    opt['encoder/backward-rnn/variant'] = 'fast'
    opt['encoder/forward-rnn/gates/weight'] = False
    opt['encoder/forward-rnn/gates/bias'] = False
    opt['encoder/forward-rnn/transform/weight'] = False
    opt['encoder/forward-rnn/transform/bias'] = True
    opt['encoder/backward-rnn/gates/weight'] = False
    opt['encoder/backward-rnn/gates/bias'] = False
    opt['encoder/backward-rnn/transform/weight'] = False
    opt['encoder/backward-rnn/transform/bias'] = True

    # decoder
    opt['decoder/init-transform/variant'] = 'fast'
    opt['decoder/annotation-transform/variant'] = 'fast'
    opt['decoder/state-transform/variant'] = 'fast'
    opt['decoder/context-transform/variant'] = 'fast'
    opt['decoder/rnn/variant'] = 'fast'
    opt['decoder/maxout/variant'] = 'fast'
    opt['decoder/deepout/variant'] = 'fast'
    opt['decoder/classify/variant'] = 'fast'
    ...
    opt['decoder/context-transform/weight'] = False
    opt['decoder/context-transform/bias'] = False
    opt['decoder/rnn/gates/weight'] = False
    opt['decoder/rnn/gates/bias'] = False
    opt['decoder/rnn/transform/weight'] = False
    opt['decoder/rnn/transform/bias'] = True
    opt['decoder/maxout/weight'] = False
    opt['decoder/maxout/bias'] = True
...
def encode_source(model, xseq, xmask):
    encode = model.sample[0]
    compute_istate = model.sample[1]

    xmask = numpy.ones(xseq.shape, 'float32')
    annot = encode(xseq, xmask)
    state, mannot = compute_istate(annot)

    return annot, state, mannot
'''
# sample from model
def sample(model, xseq, xmask, state, annot, mannot, **option):
    add_if_not_exsit(option, 'maxlen', None)
    add_if_not_exsit(option, 'minlen', None)
    ...
    for k in range(maxlen):
        ...
        if k > 0:
            last_words = numpy.array(map(lambda t: t[-1], samples))
            last_words = last_words.astype('int32')
        else:
            last_words = numpy.zeros(batch, 'int32')

        contexts = compute_context(states, xmask, annot, mannot)
        ...
        indices = numpy.argmax(random.multinomial(pvals = probs), 1)

        ymask = numpy.ones((batch,), 'float32')
        states = compute_state(indices, ymask, states, contexts)
